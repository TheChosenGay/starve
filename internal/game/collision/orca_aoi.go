...
// OrcaAOI 是 **ORCA 邻居查询专用的均匀网格**。
//
// 命名：它与 AOIGrid（感知系统）**共用"按格分桶"的思路**，但用途和取舍都不同：
//
//	            AOIGrid（感知）              OrcaAOI（本类型）
//	用途        AI 仇恨/目标选择              ORCA 找邻居
//	内容        所有 live 实体                只有会自己动的实体（有 Moveable）
//	标记        每实体标 (2r+1)² 格（O(r²)）  每实体只标自己那一格（O(1)）
//	查询        读单格（O(1)）                读自己 r 范围（O(r²)）
//	半径        2~7 格                       10 格
//
// 也就是说：AOI 把成本放在**标记**（因为"谁看得见我"必须预先铺开），
// OrcaAOI 把成本放在**查询**（因为"我附近有谁"只需临时读一圈）。
// 后者更适合 ORCA：邻居是查询方的局部信息，不需要全局铺开。
//
// 另外与 AOIGrid 的关键差别：
...
// NewOrcaAOI 建一个覆盖 (0,0)-(w,h) 的网格。
func NewOrcaAOI(w, h int, cellSize float64) *OrcaAOI {
	if cellSize <= 0 {
		cellSize = 1
	}
...
func (g *OrcaAOI) cellIndex(x, z float64) int {
	cx := int(x / g.cellSize)
	cy := int(z / g.cellSize)
...
func (g *OrcaAOI) Add(e ecs.Entity, x, z float64) {
...
func (g *OrcaAOI) removeFromCell(cell int, e ecs.Entity) {
	bucket := g.cells[cell]
...
func (g *OrcaAOI) Remove(e ecs.Entity) {
...
// Pos 返回缓存的实体位置。
func (g *OrcaAOI) Pos(e ecs.Entity) (float64, float64, bool) {
	p, ok := g.pos[e]
	return p[0], p[1], ok
}
...
func (g *OrcaAOI) Range(x, z, r float64, fn func(e ecs.Entity) bool) {
...
// Len 返回网格里的实体总数（观测用）。
func (g *OrcaAOI) Len() int { return len(g.pos) }

// Cols/Rows 返回网格尺寸（观测用）。
func (g *OrcaAOI) Cols() int { return g.cols }
func (g *OrcaAOI) Rows() int { return g.rows }

// NeighborsOrcaAOI 用网格收集邻居（方案 B 的实现）：
...
func (g *OrcaAOI) SetMotion(e ecs.Entity, m Motion) {
...
func (g *OrcaAOI) MotionOf(e ecs.Entity) (Motion, bool) {
	p, ok := g.pos[e]
	if !ok {
		return Motion{}, false
	}
...
func (g *OrcaAOI) NeighborsFromGrid(
