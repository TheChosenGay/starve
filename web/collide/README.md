...
| 模式 | 每次查询做什么 | 形状测试调用次数 |
|---|---|---|
| 不用扫描器 | 把 N 个物体**全部**喂给 `TestShapes(球, 胶囊)` | N × 查询数 |
| 用扫描器 | 先做 N 次 `AABB-AABB` 剔除，只有候选进形状测试 | 候选数（微秒级） |

所以测的是：**在命中结果完全一致的前提下，形状测试被调用了多少次、花了多少时间**。
"结果一致"不是嘴上说的——`cmd/collidewasm/broad_test.go` 里逐帧对拍两个模式的命中数。
