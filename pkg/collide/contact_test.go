...
func TestContactSphereSphere(t *testing.T) {
	...
	if !vecAlmostEq(c.Point, Vec3{0.75, 0, 0}) {
		t.Fatalf("Point = %v, want (0.75,0,0)", c.Point)
	}
	if _, ok := ContactSphereSphere(a, Sphere{C: Vec3{3, 0, 0}, R: 1}); ok {
...
func TestContactSphereAABB(t *testing.T) {
	...
	c, ok := ContactSphereAABB(Sphere{C: Vec3{2, 0.5, 0.5}, R: 1.1}, box)
	if !ok {
		t.Fatal("should be in contact")
	}
	if !vecAlmostEq(c.Normal, Vec3{1, 0, 0}) {
	...
	if !vecAlmostEq(c.Point, Vec3{1, 0.5, 0.5}) {
...
func TestContactSphereAABBInside(t *testing.T) {
	...
	c, ok := ContactSphereAABB(Sphere{C: Vec3{1, 1.8, 1}, R: 0.3}, box)
	if !ok {
		t.Fatal("sphere inside box should be in contact")
	}
	if !vecAlmostEq(c.Normal, Vec3{0, 1, 0}) {
...
func TestContactCapsuleCapsule(t *testing.T) {
	a := Capsule{A: Vec3{0, 0, 0}, B: Vec3{0, 2, 0}, R: 0.35}
	b := Capsule{A: Vec3{0.7, 0, 0}, B: Vec3{0.7, 2, 0}, R: 0.4}
	...
	far := Capsule{A: Vec3{0.8, 0, 0}, B: Vec3{0.8, 2, 0}, R: 0.4}
...
func TestContactSphereCapsule(t *testing.T) {
	cap := Capsule{A: Vec3{0, 0, 0}, B: Vec3{0, 2, 0}, R: 0.4}
	c, ok := ContactSphereCapsule(Sphere{C: Vec3{0.8, 1, 0}, R: 0.5}, cap)
	if !ok {
...
func TestContactAABBAABB(t *testing.T) {
	...
	b := AABB{Min: Vec3{1.5, 0, 0}, Max: Vec3{3.5, 2, 2}}
	...
	if _, ok := ContactAABBAABB(a, AABB{Min: Vec3{3, 0, 0}, Max: Vec3{4, 1, 1}}); ok {
