...
func TestSweepSphereSphere(t *testing.T) {
	s := Sphere{C: Vec3{0, 0, 0}, R: 0.5}
	b := Sphere{C: Vec3{5, 0, 0}, R: 1}
	h := SweepSphereSphere(s, Vec3{10, 0, 0}, b)
	if !h.Hit {
		t.Fatal("should hit")
	}
...
func TestSweepSphereAABBTunneling(t *testing.T) {
	box := AABB{Min: Vec3{4.9, 0, -1}, Max: Vec3{5.1, 2, 1}}
	s := Sphere{C: Vec3{0, 1, 0}, R: 0.5}
	motion := Vec3{10, 0, 0}
	...
	if !vecAlmostEq(h.Point, Vec3{4.9, 1, 0}) {
...
func TestSweepSphereAABBNoHit(t *testing.T) {
	box := AABB{Min: Vec3{4.9, 0, -1}, Max: Vec3{5.1, 2, 1}}
	s := Sphere{C: Vec3{0, 5, 0}, R: 0.5} // 高于盒子，扫过去碰不到
	if h := SweepSphereAABB(s, Vec3{10, 0, 0}, box); h.Hit {
...
func TestSweepFirstSphereAABB(t *testing.T) {
	boxes := []AABB{
		{Min: Vec3{3, 0, -1}, Max: Vec3{3.2, 2, 1}},
		{Min: Vec3{5, 0, -1}, Max: Vec3{5.2, 2, 1}},
		{Min: Vec3{7, 0, -1}, Max: Vec3{7.2, 2, 1}},
	}
	s := Sphere{C: Vec3{0, 1, 0}, R: 0.5}
	idx, h, ok := SweepFirstSphereAABB(s, Vec3{10, 0, 0}, boxes)
	if !ok {
...
func TestSweepRespectsRange(t *testing.T) {
	box := AABB{Min: Vec3{4.9, 0, -1}, Max: Vec3{5.1, 2, 1}}
	s := Sphere{C: Vec3{0, 1, 0}, R: 0.5}
	if h := SweepSphereAABB(s, Vec3{2, 0, 0}, box); h.Hit { // 只走 2，够不到 x=4.4
		t.Fatalf("range too short, should miss, got %+v", h)
	}
	if h := SweepSphereAABB(s, Vec3{5, 0, 0}, box); !h.Hit { // 走 5，够得到
...
func TestSweepSphereCapsule(t *testing.T) {
	body := Capsule{A: Vec3{0, 0, 0}, B: Vec3{0, 2, 0}, R: 0.4}
	bullet := Sphere{C: Vec3{-5, 1, 0}, R: 0.25}
	motion := Vec3{10, 0, 0}
	...
	if !vecAlmostEq(h.Point, Vec3{-0.4, 1, 0}) {
...
func TestSweepSphereCapsuleMiss(t *testing.T) {
	body := Capsule{A: Vec3{0, 0, 0}, B: Vec3{0, 2, 0}, R: 0.4}
	bullet := Sphere{C: Vec3{-5, 3, 0}, R: 0.25} // 高于胶囊顶端
	if h := SweepSphereCapsule(bullet, Vec3{10, 0, 0}, body); h.Hit {
